...
| isLauncher | Integer | 是否发起人（0-否/1-是） |
| productQuantity | Integer | 购买商品数量（v1.0.3新增） |
...
### v1.0.3 (2025-11-07) - 添加商品数量字段

**新功能**:
- ✅ 在MemberInfoResponse中添加`productQuantity`字段
- ✅ 团详情接口返回每个成员的购买商品数量
- ✅ 前端显示"🛒 X件商品"信息

**数据库变更**:
- ✅ `group_buy_member`表新增`product_quantity`字段
- ✅ 提供数据库迁移脚本，支持现有数据兼容
- ✅ 参团时自动保存商品数量，避免跨服务查询

**性能优化**:
- ✅ 消除N+1查询问题，直接从数据库字段获取商品数量
- ✅ 提升团详情页面的响应速度

### v1.0.2 (2025-11-01) - 支付回调优化

**新功能**:
- ✅ 团长发起拼团时支持立即参与并自动支付
- ✅ 支付回调新增余额不足错误处理
- ✅ 团长管理团队功能（提前结束拼团、移除成员、取消拼团）
