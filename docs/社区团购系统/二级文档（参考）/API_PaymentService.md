...
| **管理端支付接口** | 3个 | `/api/payment/admin` | 🟢 管理员查询、统计 |
| **总计** | **10个** | - | - |
...
```http
GET /api/payment/admin/records
```

**功能**: 管理员查询所有用户的支付/充值/退款记录（最核心的管理端接口）
...
| payType | Integer | 否 | 支付方式（1-微信；2-支付宝；3-余额） |
| payStatus | Integer | 否 | 支付状态（0-失败；1-成功） |
| recordType | String | 否 | 记录类型（recharge/payment/refund） |
| keyword | String | 否 | 搜索关键词（订单号/交易号） |
    ...
    "content": [
      {
        ...
        "payStatus": 1,
        "transactionId": "RECHARGE_1698825000123",
        "createTime": "2025-11-06T16:30:00"
      },
      {
        "payId": 9,
        "userId": 2,
        ...
        "payStatus": 1,
        "transactionId": "BALANCE_1698825000456",
        "createTime": "2025-11-06T16:25:00"
      },
      {
        "payId": 8,
        "userId": 3,
        ...
        "payStatus": 1,
        "transactionId": "REFUND_1698825000789",
        "createTime": "2025-11-06T16:20:00"
      }
    ],
    "pageable": {
      "pageNumber": 0,
...
1. **记录类型（recordType）**:
   - `recharge`: 充值记录（orderId为null，amount为正）
   - `payment`: 支付记录（orderId不为null，amount为正）
   - `refund`: 退款记录（amount为负）

2. **关键词搜索（keyword）**:
   - 支持订单号（orderId）模糊搜索
   - 支持交易号（transactionId）模糊搜索

3. **日期范围**:
   - startDate: 查询此日期及之后的记录
   - endDate: 查询此日期及之前的记录
...
```json
{
  ...
  "data": {
    ...
    "totalAmount": 5280.50,
    "rechargeCount": 45,
    "rechargeAmount": 4500.00,
    "paymentCount": 95,
    "paymentAmount": 2850.50,
    "refundCount": 10,
    "refundAmount": -2070.00,
    "successRate": 92.67,
    "balancePayRate": 100.00,
    "wechatPayRate": 0.00,
    "alipayPayRate": 0.00
  },
  "timestamp": "2025-11-06T16:30:00"
...
|-----|------|------|
| total | Long | 总记录数 |
| totalAmount | BigDecimal | 总交易金额（充值 + 支付 + 退款） |
| rechargeCount | Long | 充值记录数 |
| rechargeAmount | BigDecimal | 充值总额 |
| paymentCount | Long | 支付记录数（订单支付） |
| paymentAmount | BigDecimal | 支付总额（订单支付） |
| refundCount | Long | 退款记录数 |
| refundAmount | BigDecimal | 退款总额（负数） |
| successRate | Double | 支付成功率（%） |
| balancePayRate | Double | 余额支付占比（%） |
| wechatPayRate | Double | 微信支付占比（%） |
| alipayPayRate | Double | 支付宝支付占比（%） |
...
```http
GET /api/payment/admin/user/{userId}/records
```
...
#### 测试1：查询所有支付记录
```bash
GET http://localhost:8066/api/payment/admin/records?page=0&size=20
...
#### 测试2：按条件筛选
```bash
GET http://localhost:8066/api/payment/admin/records?recordType=recharge&payStatus=1
...
#### 测试3：关键词搜索
```bash
GET http://localhost:8066/api/payment/admin/records?keyword=BALANCE_

# 预期结果：
# - 返回所有交易号包含"BALANCE_"的记录
```

#### 测试4：日期范围筛选
```bash
GET http://localhost:8066/api/payment/admin/records?startDate=2025-11-01&endDate=2025-11-06

# 预期结果：
# - 返回2025-11-01到2025-11-06之间的记录
```

#### 测试5：支付统计
```bash
GET http://localhost:8066/api/payment/admin/statistics
...
#### 测试6：指定时间范围统计
```bash
GET http://localhost:8066/api/payment/admin/statistics?startDate=2025-11-01&endDate=2025-11-06
...
#### 测试7：查询指定用户支付记录
```bash
GET http://localhost:8066/api/payment/admin/user/1/records
...
**管理端接口安全措施**:
1. 需要管理员权限验证（建议在Gateway层统一处理）
2. 所有管理端接口路径以 `/admin` 开头
3. 建议添加IP白名单
4. 敏感操作需要二次确认
...
**A**: 
- 建议在Gateway层添加管理员权限过滤器
- 检查JWT中的角色信息（role=admin）
- 管理端接口统一使用 `/api/*/admin/*` 路径前缀
- PaymentService专注于业务逻辑，权限由Gateway统一管理

### Q7: 管理端查询大量数据时如何优化性能？

**A**: 
- 使用Spring Data JPA的Specification动态查询
- 支持分页查询（Page/Pageable）
- 合理使用索引（create_time, user_id, order_id）
- 考虑添加缓存（Redis）
- 统计数据可以定期计算并缓存
...
### v1.1.0 (2025-11-06)
- ✅ 新增管理端查询接口 `GET /api/payment/admin/records`
- ✅ 新增管理端统计接口 `GET /api/payment/admin/statistics`
- ✅ 新增查询指定用户记录接口 `GET /api/payment/admin/user/{userId}/records`
- ✅ 添加JpaSpecificationExecutor支持动态查询
- ✅ 更新API文档结构和测试用例
