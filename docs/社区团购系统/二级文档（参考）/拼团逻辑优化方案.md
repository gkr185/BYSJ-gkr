...
| 业务问题 | 选定方案 | 说明 |
|---------|---------|------|
| 拼团模式 | **社区配送特色** | 团长发起拼团，用户参与，订单按团归属配送 |
| **发起权限** ⭐v3.0 | **仅团长可发起拼团** | 普通用户只能参与拼团，不能发起；团长role=2才能发起 |
| 支付时机 | **参团时立即支付** | 避免成团后用户不支付，符合主流电商逻辑 |
| 多团支持 | **一个活动可以有多个团** | 需要"活动+团实例"两层结构 |
| 团长归属 | **团归属团长** ⭐v3.0更新 | 团长发起团时，团自动关联该团长；用户参团时选择团，订单归属该团的团长 |
| 拼团失败 | **24小时自动退款** | 退款到用户账户余额（`user_account`） |
| 中途退出 | **成团前可退，成团后不可退** | 保护各方利益 |
| 团长角色 | **配送点负责人** | 团长发起拼团并负责配送和团员管理 |
...
- **社区（Community）**：地理上的社区，用于优先推荐团购活动
- **社区归属**：团长归属一个社区，团长发起的团自动关联该社区
- **跨社区拼团**：用户可以参与其他社区的团，但优先显示本社区的团
- **社区申请**：团长申请时可选择已有社区或申请新社区，需管理员审核

#### 社区表设计

详见《数据库设计说明文档.md》第2.1节 - 社区表（community）
...
详见《数据库设计说明文档.md》第2.2节 - 社区申请表（community_application）

**核心字段**：
```sql
community_application (
  application_id BIGINT PRIMARY KEY,
  applicant_id BIGINT,  -- 申请人（团长）
  community_name VARCHAR(100),
  province, city, district, address,
  status TINYINT DEFAULT 0,  -- 0待审核/1已通过/2已拒绝
  approved_community_id BIGINT,  -- 审核通过后的社区ID
  auditor_id BIGINT,
  audit_time DATETIME
)
```
...
1. **团长申请时选择或申请新社区**：
   - 可选择已有社区
   - 可申请新社区（需填写社区信息，等待管理员审核）

2. **用户添加地址时自动匹配最近社区**（≤5km）：
   - 通过经纬度计算距离
   - 自动设置用户的 `community_id`

3. **团长发起团时，团自动关联团长的社区**：
   - `group_buy_team.community_id` = `sys_user.community_id`（团长的社区）
...
- **含义**：管理与团长固定绑定的团员
- **数据表**：`group_member`（团员绑定关系表）
- **关系**：一个用户绑定一个团长，形成固定关系

#### v3.0 定义（当前）

- **含义**：管理团长发起的团的参与成员
- **数据表**：`group_buy_member`（参团记录表）
- **关系**：用户参团时选择团，团归属团长，团长查看自己发起的团的成员
...
```
1. 团长选择"苹果3人拼团活动"（activity_id=1）
2. 系统验证发起人是团长（role=2）
   → 不是团长：提示"仅团长可发起拼团"
   → 是团长：继续
3. 系统创建团实例（team_id=5001，team_no=T20251027001）
   → 关联活动ID、发起人（团长）、团长、社区ID、成团人数、过期时间
   → launcher_id = leader_id（团长发起）
   → community_id = 团长的社区ID（自动关联）
4. 系统创建订单（如果团长参与，order_id=8001，状态=待支付）
5. 团长支付（如果团长参与）
6. 系统记录参团（team_id=5001, user_id=团长ID, order_id=8001, is_launcher=1）
...
CREATE TABLE group_buy_team (
  ...
  launcher_id BIGINT NOT NULL COMMENT '发起人用户ID（团长）',
  leader_id BIGINT NOT NULL COMMENT '归属团长ID（配送团点）',
  community_id BIGINT DEFAULT NULL COMMENT '归属社区ID（团长的社区）⭐v3.0新增',
...
**字段说明**：
- `launcher_id`：发起人ID，在v3.0中仅团长可发起，所以 launcher_id = leader_id
- `leader_id`：归属团长ID，用于订单配送归属
- `community_id`：归属社区ID，团长发起时自动设置为团长的社区，用于优先推荐
...
**关键变更**：
- ❌ v2.0: 用户可以发起拼团
- ✅ v3.0: **仅团长可以发起拼团**（role=2）
- ❌ v2.0: 检查用户是否已绑定团长
- ✅ v3.0: 验证发起人是团长，自动关联团长的社区
...
sequenceDiagram
    ...
    alt 不是团长
        ...
        DB-->>B: role = 2, 返回 community_id
        
        %% Step 3: 创建团实例（自动关联社区）
        B->>DB: INSERT INTO group_buy_team (launcher_id=leader_id, community_id)
        DB-->>B: 返回 team_id
        ...
        alt 团长参与拼团
            ...
            %% Step 5: 记录参团
            B->>DB: INSERT INTO group_buy_member (is_launcher=1)
            DB-->>B: 插入成功
        end
...
**1. group_member 表（团员绑定关系表）**

- **原用途**：管理团长与团员的固定绑定关系
- **废弃原因**：v3.0改为动态关联，用户参团时选择团，无需固定绑定
- **替代方案**：使用 `group_buy_member` 表的参团记录
- **数据库操作**：
  ```sql
  -- 备份后删除
  RENAME TABLE group_member TO group_member_backup_v2;
  -- 确认无问题后可删除备份
  DROP TABLE IF EXISTS group_member_backup_v2;
  ```

**2. 用户绑定团长流程**

- **原流程**：用户加入团长 → 固定绑定 → 所有订单归属该团长
- **新流程（v3.0）**：用户参团时选择团 → 团归属团长 → 订单归属该团的团长
- **优势**：
  - 用户可以参与多个不同团长的团
  - 更灵活的团购模式
  - 符合实际业务场景
...
- **原逻辑**：用户可以发起拼团（类似拼多多）
- **新逻辑（v3.0）**：仅团长可以发起拼团（role=2）
- **原因**：
  - 符合社区团购业务模式
  - 团长作为社区服务点负责人，负责发起和配送
  - 普通用户参与拼团即可
...
|-------|------|------|
| 发起权限 | ❌ 用户可发起 | ✅ 仅团长可发起 |
| 用户-团长关系 | ❌ 固定绑定（group_member） | ✅ 动态关联（选团参团） |
| 社区机制 | ❌ 未引入 | ✅ 引入社区表和申请流程 |
| 团员管理 | ❌ 管理固定绑定的团员 | ✅ 管理团的参与成员 |
| 数据表 | group_member, group_buy_join | group_buy_team, group_buy_member, community |
...
1. **备份数据库**：
   ```bash
   mysqldump -u root -p community_group_buy > backup_v2.sql
   ```
...
| 发起人 | Launcher | 创建团的用户（v3.0仅团长） |
| 团长 | Leader | 配送点负责人，发起拼团 |
| 团员 | Member | 参与团长发起的团的成员（v3.0新定义） |
| 社区 | Community | 地理上的社区，用于优先推荐（v3.0新增） |
