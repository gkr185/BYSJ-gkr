...
- **微服务隔离**: 每个微服务独立数据库，服务间通过API调用 ⭐v4.0新增

### 1.2 架构演进

#### 单体数据库（v1.0 - v3.0）
- 1个数据库：`community_group_buy`
- 19张表，所有服务共享
- 物理外键：33个
...
系统采用**微服务架构**，按业务域拆分为 **7个独立数据库**，每个微服务对应一个数据库。

| 数据库名称 | 对应微服务 | 端口 | 表数 | 物理外键 | 包含表 |
|-----------|-----------|------|------|---------|--------|
| **user_service_db** | UserService | 8061 | 5张 | 4个 | sys_user、user_address、user_account、user_feedback、sys_operation_log |
| **product_service_db** | ProductService | 8062 | 2张 | 1个 | product_category、product |
| **groupbuy_service_db** | GroupBuyService | 8063 | 3张 | 2个 | group_buy、group_buy_team、group_buy_member |
| **order_service_db** | OrderService | 8065 | 3张 | 1个 | shopping_cart、order_main、order_item |
| **payment_service_db** | PaymentService | 8066 | 1张 | 0个 | payment_record |
| **delivery_service_db** | DeliveryService | 8067 | 1张 | 0个 | delivery |
| **leader_service_db** | LeaderService | 8068 | 4张 | 2个 | group_leader_store、commission_record、community、community_application |
...
#### 策略1：数据冗余（快照设计）
**应用场景**: 订单相关数据
- `order_item` 表保存 `product_name`、`product_img`、`price`（下单时快照）
- 避免查询订单时频繁跨库调用商品服务

**示例**:
```sql
-- 订单服务数据库内查询，无需跨库
SELECT o.order_sn, oi.product_name, oi.price, oi.quantity
FROM order_main o
...
public void joinGroupBuy(Long userId, Long teamId) {
    ...
    try {
        ...
        orderServiceClient.createOrder(..., sagaId);
...
#### 策略4：应用层校验
**应用场景**: 替代物理外键约束
- 删除数据前，先调用其他服务检查是否存在关联
- 定期执行数据一致性检查脚本

**示例**:
```sql
-- 定期检查孤立数据（每天凌晨执行）
SELECT order_id, user_id FROM order_service_db.order_main 
WHERE user_id NOT IN (
  SELECT user_id FROM user_service_db.sys_user
);
```
...
| 优势 | 说明 |
|------|------|
| **服务独立性** | 每个服务独立部署、升级、回滚，互不影响 |
| **故障隔离** | 单个数据库故障不影响其他服务正常运行 |
| **性能优化** | 可针对单个数据库进行索引优化、分区表设计 |
| **技术异构** | 不同服务可使用不同数据库（如引入MongoDB） |
| **团队协作** | 不同团队独立开发，减少数据库锁冲突 |
| **水平扩展** | 高负载服务独立扩容（如订单服务读写分离） |
...
### 3.3 社区申请表 (community_application) ⭐v3.0新增
...
### 3.4 用户基础信息表 (sys_user) ⭐v3.0增加社区字段
...
### 3.10 团长团点表 (group_leader_store) ⭐v3.0增加社区字段
...
### 3.11 团员关系表 (group_member) ❌v3.0废弃
...
### 3.13 团实例表 (group_buy_team) ⭐v3.0优化
...
### 3.14 参团记录表 (group_buy_member) ⭐新增核心表
...
### 3.21 系统操作日志表 (sys_operation_log) ⭐已扩展
...
| 外键名 | 所属数据库 | 子表 | 子表字段 | 父表 | 父表字段 | DELETE策略 | UPDATE策略 |
|--------|-----------|------|---------|------|---------|-----------|-----------|
| fk_address_user | user_service_db | user_address | user_id | sys_user | user_id | CASCADE | CASCADE |
| fk_account_user | user_service_db | user_account | user_id | sys_user | user_id | CASCADE | CASCADE |
| fk_feedback_user | user_service_db | user_feedback | user_id | sys_user | user_id | CASCADE | CASCADE |
| fk_log_user | user_service_db | sys_operation_log | user_id | sys_user | user_id | SET NULL | CASCADE |
| fk_product_category | product_service_db | product | category_id | product_category | category_id | SET NULL | CASCADE |
| fk_team_activity | groupbuy_service_db | group_buy_team | activity_id | group_buy | activity_id | CASCADE | CASCADE |
| fk_member_team | groupbuy_service_db | group_buy_member | team_id | group_buy_team | team_id | CASCADE | CASCADE |
| fk_item_order | order_service_db | order_item | order_id | order_main | order_id | CASCADE | CASCADE |
| fk_store_community | leader_service_db | group_leader_store | community_id | community | community_id | SET NULL | CASCADE |
| fk_app_community | leader_service_db | community_application | approved_community_id | community | community_id | SET NULL | CASCADE |

#### 删除的跨库外键（应用层校验，22个）❌

| 原外键名 | 子表（数据库） | 子表字段 | 父表（数据库） | 父表字段 | 替代方案 |
|---------|---------------|---------|--------------|---------|---------|
| ~~fk_user_community~~ | sys_user (user_service_db) | community_id | community (leader_service_db) | community_id | 服务调用验证 |
| ~~fk_app_applicant~~ | community_application (leader_service_db) | applicant_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_app_auditor~~ | community_application (leader_service_db) | auditor_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_store_leader~~ | group_leader_store (leader_service_db) | leader_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_group_product~~ | group_buy (groupbuy_service_db) | product_id | product (product_service_db) | product_id | 服务调用验证 |
| ~~fk_team_launcher~~ | group_buy_team (groupbuy_service_db) | launcher_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_team_leader~~ | group_buy_team (groupbuy_service_db) | leader_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_team_community~~ | group_buy_team (groupbuy_service_db) | community_id | community (leader_service_db) | community_id | 服务调用验证 |
| ~~fk_member_user~~ | group_buy_member (groupbuy_service_db) | user_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_member_order~~ | group_buy_member (groupbuy_service_db) | order_id | order_main (order_service_db) | order_id | 服务调用验证 |
| ~~fk_cart_user~~ | shopping_cart (order_service_db) | user_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_cart_product~~ | shopping_cart (order_service_db) | product_id | product (product_service_db) | product_id | 服务调用验证 |
| ~~fk_cart_activity~~ | shopping_cart (order_service_db) | activity_id | group_buy (groupbuy_service_db) | activity_id | 服务调用验证 |
| ~~fk_order_user~~ | order_main (order_service_db) | user_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_order_leader~~ | order_main (order_service_db) | leader_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_order_address~~ | order_main (order_service_db) | receive_address_id | user_address (user_service_db) | address_id | 快照设计（冗余地址） |
| ~~fk_order_delivery~~ | order_main (order_service_db) | delivery_id | delivery (delivery_service_db) | delivery_id | 服务调用验证 |
| ~~fk_item_product~~ | order_item (order_service_db) | product_id | product (product_service_db) | product_id | 快照设计（冗余商品名称/图片/价格） |
| ~~fk_item_activity~~ | order_item (order_service_db) | activity_id | group_buy (groupbuy_service_db) | activity_id | 服务调用验证 |
| ~~fk_delivery_leader~~ | delivery (delivery_service_db) | leader_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_payment_user~~ | payment_record (payment_service_db) | user_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_payment_order~~ | payment_record (payment_service_db) | order_id | order_main (order_service_db) | order_id | 服务调用验证 |
| ~~fk_commission_leader~~ | commission_record (leader_service_db) | leader_id | sys_user (user_service_db) | user_id | 服务调用验证 |
| ~~fk_commission_order~~ | commission_record (leader_service_db) | order_id | order_main (order_service_db) | order_id | 服务调用验证 |

**说明**：
- ✅ 单库内外键继续使用物理约束，保证数据库层面的引用完整性
- ❌ 跨库外键已删除，通过应用层代码校验：
  - **服务调用验证**：创建数据前调用关联服务API验证ID有效性
  - **快照设计**：冗余关联数据（如order_item保存product_name、product_img、price）
  - **定期检查脚本**：每天凌晨执行数据一致性检查，发现并修复孤立数据
...
### v4.0 (2025-10-29) - 微服务数据库拆分 ⭐重大变更
**架构调整**：
1. ⭐ **微服务数据库拆分**：
   - 单体数据库 → 7个独立数据库
   - 每个微服务对应一个数据库
   - 数据库列表：user_service_db、product_service_db、groupbuy_service_db、order_service_db、payment_service_db、delivery_service_db、leader_service_db
...
3. ⭐ **跨库关联处理**：
   - 数据冗余（快照设计）：order_item保存商品快照
   - 服务间HTTP调用：OpenFeign声明式客户端
   - 分布式事务：Saga模式 + 补偿机制
   - 应用层校验：替代物理外键约束
...
**影响**：
- 后端需要新增服务间调用客户端（OpenFeign）
- 前端无需修改（API接口保持不变）
- 需要配置7个数据库连接（application.yml）
- 需要实现分布式事务协调器（SagaCoordinator）

**迁移步骤**：
1. 执行7个独立SQL脚本创建数据库
2. 修改各微服务的数据库配置
3. 实现服务间调用接口（UserServiceClient等）
4. 实现补偿事务逻辑
5. 部署并测试各微服务

**相关文档**：
- SQL脚本目录：`community-group-buy-backend/sql/`
- 拆分对比报告：`docs/社区团购系统/数据库微服务拆分对比报告.md`
- 使用指南：`community-group-buy-backend/sql/README.md`
...
**维护人**: AI Assistant  
**联系方式**: 参见开题报告
**SQL脚本路径**: `community-group-buy-backend/sql/`  
**相关文档**: `docs/社区团购系统/数据库微服务拆分对比报告.md`
