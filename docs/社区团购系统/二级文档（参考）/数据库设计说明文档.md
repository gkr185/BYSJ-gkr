...
### 2.10 团实例表 (group_buy_team) ⭐新增核心表
...
| launcher_id | bigint | - | NO | - | 发起人用户ID |
| leader_id | bigint | - | NO | - | 归属团长ID（配送团点） |
...
2. `launcher_id` 为发起拼团的用户
3. `leader_id` 为订单配送的团长（从 `group_member` 表查询用户绑定的团长）
4. `required_num` 从活动复制，支持动态调整
5. `current_num` 实时统计参团人数
6. `expire_time` 默认为创建后24小时，超时未成团则标记为失败
7. 成团条件：`current_num >= required_num`
8. 定时任务每小时检查过期团，自动退款
...
| group_buy_team | team_id | PRIMARY | BTREE | ⭐新增
| group_buy_member | member_id | PRIMARY | BTREE | ⭐新增
...
| group_buy_team | uk_team_no | team_no | 团号唯一 ⭐新增 |
| group_buy_member | uk_team_user | team_id, user_id | 一个用户不能重复参加同一个团 ⭐新增 |
...
- `group_buy.idx_status`: 按状态筛选活动 ⭐新增
- `group_buy_team.idx_activity_id`: 查询活动的所有团 ⭐新增
- `group_buy_team.idx_team_status`: 按团状态筛选 ⭐新增
- `group_buy_member.idx_team_id`: 查询团的所有成员 ⭐新增
...
2. 用户发起拼团时，自动关联绑定的团长（group_member表）
3. 参团时立即生成订单并支付
4. 成团前可退出（删除记录），成团后不可退出
5. 24小时未成团自动退款到用户账户
6. 支持团号分享（team_no）
```
